...
class TwoArmPegInHoleCustom(TwoArmEnv):
    """
        ...
        if self.env_configuration == "bimanual":
            ...
            if self.env_configuration == "single-arm-opposed":
                ...
                for robot, rotation in zip(self.robots, (np.pi / 2, -np.pi / 2)):
                    ...
                    random_values = np.random.uniform(-0.1, 0.1, 3)
                    # random_offset = np.append(random_values, 0)
                    xpos = T.euler2mat(rot) @ np.array(xpos) + random_values
